...
def loadTestData(filename):
    ...
    #Opening the file specified at the bottom
    ...
    for line in file:
        ...
        try:
            ...
            #Appending the features to the test data and test label arrays
            if label == 0:
                testData.append([float(duration), protocolDictionary[protocol], int(sourcePort),                    int(destinationPort), sourceIP, destinationIP, int(totP),                        int(totB), stateDictionary[state]])
    ...
    #Pickling the file
    picklefile = '../dataset/testData1.pickle' #Change this to change the name of the file
    file = open(picklefile, 'wb')
    pickle.dump([np.array(testData), np.array(testLabel)], file)
    
    print("Test data created")

    
if __name__ == "__main__":
    loadTestData('../dataset/flowdata1.binetflow') #Change this to run on a differnt .binetflow file